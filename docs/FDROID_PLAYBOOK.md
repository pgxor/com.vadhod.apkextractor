...
### 2.3 Disable the dependency-metadata signing block
...
AGP embeds a **Google-encrypted blob** into the APK Signing Block by default. `scanner.py` lists it as
block ID `0x504B4453` with the comment *"Dependencies metadata generated by Gradle and encrypted by a
Google Play signing key"*, and the `check apk` job fails on it:

```
CRITICAL: Found 1 problems in .../com.vadhod.apkextractor_3.binary.apk
ERROR Found extra signing block 'Dependency metadata' in ...
```

This is **not** caught by `fdroid build` — that job can pass, reproducibility and all, and `check apk`
still fails afterwards. It is opaque, proprietary, and not reproducible, so there is no route other than
turning it off at the source.

Set this before you ever build a release you intend to publish. Changing it later means a new commit, a
new tag, a new release APK, and another review round — which is exactly what it cost on `!44998`.

### 2.4 Remove the foojay toolchain plugin
